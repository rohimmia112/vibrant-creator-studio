    ...
    return (
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
          ...
          />
          <Line type="monotone" dataKey={Object.keys(data[0])[1]} stroke={color} strokeWidth={2} dot={{ r: 3 }} />
        </LineChart>
      </ResponsiveContainer>
    );
  }
    ...
    return (
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
          ...
          />
          <Bar dataKey={Object.keys(data[0])[1]} fill={`url(#colorGradient-${color.replace('#', '')})`} />
        </BarChart>
      </ResponsiveContainer>
    );
  }
            ...
            <CarouselContent className="-ml-4">
              {testimonialItems.map((item) => (
                <CarouselItem key={item.id} className="pl-4 md:basis-1/2 lg:basis-1/2">
                  <Card className="overflow-hidden border border-purple-100 shadow-sm hover:shadow-md transition-shadow duration-300 h-[360px]">
                      ...
                      <div className="flex flex-col h-full">
                        ...
                        <div className="flex flex-wrap gap-4 mb-6">
                          {item.metrics.map((metric, index) => (
                            <div key={index} className="bg-gray-50 rounded-lg p-3 min-w-[80px]">
